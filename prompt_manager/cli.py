...
def get_manager():
    ...
    manager = PromptManager("", memory_path=Path(project_dir).absolute() / "prompt_manager_data")
    return manager
...
@cli.command()
@click.option("--title", required=True, help="The task title")
@click.option("--description", required=True, help="The task description")
@click.option("--template", required=False, default="", help="Task template")
...
def add_task(title, description, template, priority):
    """Add a new task."""
...
@cli.command()
@click.option("--title", required=True, help="The task title")
@click.option("--status", required=True, type=click.Choice([s.value for s in TaskStatus], case_sensitive=False), help="Task status")
def update_progress(title, status):
    """Update task progress."""
    click.echo(get_llm_guidance("update_progress"))
    try:
        manager = get_manager()
        task = manager.update_task_status(title, status.upper())
