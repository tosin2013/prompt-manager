...
class LLMGuidance:
    ...
    @staticmethod
    def get_command_guidance(
        command: str, guidance_data: Optional[dict] = None
    ) -> str:
        """Get guidance for a specific command."""
        if command.startswith("debug_"):
            return LLMGuidance._format_debug_guidance(command, guidance_data)
        ...
        return LLMGuidance.format_guidance(
            LLMGuidance._format_guidance(guidance)
        )

    @staticmethod
    def _format_debug_guidance(command: str, guidance_data: dict) -> str:
        """Format guidance for debug commands."""
        if not guidance_data:
            return "No specific guidance available"

        # Build guidance message
        guidance = [f"# {command.replace('_', ' ').title()}\n"]

        # Add principles section
        if "principles" in guidance_data:
            guidance.append("\n## Key Debugging Principles")
            for principle in guidance_data["principles"]:
                guidance.append(f"- {principle}")

        # Add steps or layers section
        if "steps" in guidance_data:
            guidance.append("\n## Debug Steps")
            for step in guidance_data["steps"]:
                guidance.append(f"- {step}")
        elif "layers" in guidance_data:
            guidance.append("\n## Analysis Layers")
            for layer in guidance_data["layers"]:
                guidance.append(f"- {layer}")

        # Add general note
        guidance.append("\n## Note")
        guidance.append(
            "Remember to document all observations and changes during the debugging process."
        )
...
"""
    ...
    @staticmethod
    def format_guidance(guidance: str) -> str:
